...
PYBIND11_MODULE(_engine, m)
{
    ...
    py::class_<PointInit>(m, "PointInit")
        ...
        .def("__repr__", [](const PointInit &p)
             { return "<PointInit x=" + std::to_string(p.x) + " y=" + std::to_string(p.y) + " z=" + std::to_string(p.z) + " pinned=" + (p.pinned ? "True" : "False") + ">"; });
    ...
    py::class_<SpringInit>(m, "SpringInit")
        ...
        .def("__repr__", [](const SpringInit &s)
             { return "<SpringInit a=" + std::to_string(s.a) + " b=" + std::to_string(s.b) + " k=" + std::to_string(s.stiffness) + " L=" + std::to_string(s.rest_length) + ">"; });

    // bind_engine.cpp additions
    py::class_<SimThread>(m, "SimThread")
        ...
        return std::make_unique<SimThread>(
            std::move(solver),
            physics_fps,
            sub_steps
        ); }),
             ...
             py::arg("scale") = 1.0f,
             py::arg("physics_fps") = 120,
             py::arg("sub_steps") = 5)

        .def("start", &SimThread::start)
        .def("stop", &SimThread::stop)
        .def("set_stiffness", &SimThread::set_stiffness)
        .def("set_pressure", &SimThread::set_pressure)
        .def("reset", &SimThread::reset)
        .def("is_running", &SimThread::is_running)
        .def("num_points", &SimThread::num_points)

        .def("get_pos", [](SimThread &self) -> py::array_t<float>
             { return py::array_t<float>(
                   {self.num_points(), 3},
                   {3 * sizeof(float), sizeof(float)},
                   self.pos_data(),
                   py::cast(self)); }, py::return_value_policy::reference_internal, "Zero-copy (N,3) float32 view into the physics position buffer.");
    ...
    py::class_<Solver>(m, "Solver",
        ...
        .def("set_pos", [](Solver &self, py::array_t<float, py::array::c_style> arr)
             {
        auto r = arr.unchecked<2>();
        ...
        std::copy(arr.data(), arr.data() + self.num_points() * 3,
                  self.pos.begin()); }, "Overwrite positions with a (N,3) float32 array.");
    ...
    m.def("calc_mesh_volume", [](py::array_t<float, py::array::c_style> pos_arr, py::array_t<int32_t, py::array::c_style> faces_arr) -> float
          {
            ...
            ); }, py::arg("pos"), py::arg("faces"), "Compute signed mesh volume (divergence theorem).");
}
