...
namespace breast::engine
{
    ...
    public:
        ...
        // ── Step — runs Verlet + springs + pressure ───────────────────────────
        // Collision is intentionally NOT called here.
        // SimThread calls collision::World::resolve() before this.
        void update(float dt);

        // ── Decomposed step — for SimThread to interleave collision ───────────
        // Call order each sub-step:
        //   1. integrate(dt)          — Verlet
        //   2. collision resolve      — caller's responsibility
        //   3. solve_constraints(dt)  — springs + pressure + pin enforcement
        ...
        // ── Pinned access for collision module ────────────────────────────────
        const bool *pinned_data() const
        {
            return reinterpret_cast<const bool *>(pinned_.data());
        }

        int num_pinned() const { return static_cast<int>(pinned_.size()); }
