...
namespace breast::engine
{
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    // update — convenience wrapper (Verlet + collision-less constraints)
    // Used when SimThread collision is disabled or for standalone use.
    // ─────────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    // solve_constraints — springs + pressure + pin enforcement + stability check
    // ─────────────────────────────────────────────────────────────────────────────
    void Solver::solve_constraints()
    {
        if (is_exploded)
            return;
...
#pragma omp parallel for schedule(static) if (S > 512)
        for (int s = 0; s < S; ++s)
        {
            ...
            const float k = spring_k_[s] * stiffness * 0.5f * (1.f - rest_len_[s] / dist);
